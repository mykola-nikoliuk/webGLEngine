		...
		return {
			...
			findDependencies: function( fileName ) {
				...
				try {
					...
					throw new ReferenceError( "`" + fileName + "` appears to be invalid JavaScript. Error: " + e );
				}
				...
				var
						...
						checkDepForLoopRecursively = function( filename ) {
							...
							dependencyMap[ filename ].forEach(function( dep ){
								...
								if ( circuit.depFiles.indexOf( dep.filename ) !== -1 ) {
									var idx = circuit.depFiles.indexOf( dep.filename );
									console.log("`" + dep.filename + "` is required recursively and creates an infinite loop");
									console.log("first: " + circuit.srcFiles[idx]);
									console.log("second: " + filename);
									throw new ReferenceError( "`" + dep.filename +
										"` is required recursively and creates an infinite loop, file: " + srcFilename );
								}
								//var index = circuit.srcFiles.length;
								circuit.depFiles.push( dep.filename );
								circuit.srcFiles.push( filename );
								checkDepForLoopRecursively( dep.filename );
								// Remove this level adding
/*
								circuit.depFiles.splice(index, 1);
								circuit.srcFiles.splice(index, 1);
*/
								do {
									var index = circuit.depFiles.indexOf(dep.filename);
									if(index >= 0) {
										circuit.depFiles.splice(index, 1);
										circuit.srcFiles.splice(index, 1);
									}
								} while(index >= 0);
								circuit.depFiles = circuit.depFiles.filter(function( fname ){
									return fname !== dep.filename;
								});
							});
						};
				circuit = {
					depFiles : [srcFilename ],
					srcFiles : []
				};
