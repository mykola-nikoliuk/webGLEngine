...
export default class Mesh extends LinkedTransformations {
        ...
        for (material in this._faces) {
            if (this._faces.hasOwnProperty(material)) {
                ...
                for (i = 0; i < this._faces[material].length; i++) {
                    ...
                    Mesh._fixNormals(normals, positions, this._faces[material][i], counter - 3);
                }
            }
        }
    ...
    public callback(callback: Callback): Mesh {
