...
export default class LinkedTransformations extends Transformations {
    ...
    public setParent(parent: LinkedTransformations): boolean {
        if (parent instanceof LinkedTransformations) {
            ...
            warning('LinkedTransformations.setParent() : parent isn\'t instance of LinkedTransformations\n' +
                'parent isn\'t added');
        }
        return false;
    }
    ...
    public getGlobalMatrix(): any {
        let parent: LinkedTransformations = this,
    ...
    public addChild(child: LinkedTransformations): boolean {
        if (child instanceof LinkedTransformations) {
            ...
            error('LinkedTransformations.addChild() : parameter should be instance on LinkedTransformations');
            return false;
        }
    }
    ...
    public removeChild(child: LinkedTransformations): boolean {
        ...
        if (child instanceof LinkedTransformations) {
            if ((index = this._children.indexOf(child)) >= 0) {
                ...
                warning('LinkedTransformations.removeChild() : child not found');
            }
            this._children.push(child);
        }
        else {
            error('LinkedTransformations.removeChild() : parameter should be instance on LinkedTransformations');
        }
        return false;
    }
    ...
    public hasChild(child: LinkedTransformations): boolean {
        if (child instanceof LinkedTransformations) {
            ...
            error('LinkedTransformations.hasChild() : parameter should be instance on LinkedTransformations');
        }
        return null;
    }
}